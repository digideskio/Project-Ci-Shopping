...
<div class="article">
    ...
    <div class="content">
        <ul>
            <?php
            foreach ($data as $v) {
                $date = date_create($v['datecreated']);
                if ($v['img'] != "[]") {
                    $img = $v['img'];
                    $img = str_replace("[", "", $img);
                    $img = str_replace("]", "", $img);
                ...
                echo '<li>';
                echo '<h3><a href="' . base_url() . 'bai-viet/' . $url . '_post' . $v['id'] . '.html">' . $v['title'] . '</a></h3>';
                echo '<div class="date"><b>Ngày đăng</b><br /><small>' . date_format($date, "d-m-Y") . '</small></div>';
                echo '<div class="author"><b>Người đăng</b><br /><small>' . $v['fullname'] . '</small></div>';
                echo '<div class="view"><b>Lượt xem</b><br /><small>' . $v['view'] . '</small></div>';
                echo '<a href="' . base_url() . 'bai-viet/' . $url . '_post' . $v['id'] . '.html"><img alt="' . $v['title'] . '" src="' . base_url() . 'upload/' . $img . '"></a>';
                echo '<p>' . substr($v['content'], 0, 500) . '...</p>';
                echo '</li>';
            }
            ?>
        </ul>
        <div class="url-link"><?php echo $link; ?></div>
        <div>
        </div>
        <?php
