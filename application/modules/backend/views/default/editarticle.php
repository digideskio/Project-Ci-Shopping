...
<div class="form-group has-feedback">
    <input type="text" class="form-control title" placeholder="Tiêu đề bài viết" value="<?php echo $article['title']; ?>" >
</div>
<div class="form-group has-feedback upload">
    <?php if($article['img'] !="[]"){?>
    <img width="200px" src="<?php echo base_url(); ?>upload/<?php echo str_replace($array=array('[', ']', '"'),"",$article['img']); ?>" >
    <?php }else{ ?>
    <img width="200px" src="<?php echo base_url(); ?>upload/September2014113421AMno-image.jpg" >
    <?php } ?>
</div>
<div class="form-group has-feedback">
    <p>
        <textarea class="ckeditor content" cols="80" id="editor2" rows="20"><?php echo $article['content']; ?>
        </textarea>
    </p>
</div>
...
<div class="form-group has-feedback">
    <select class="span2 form-control public">
        <option <?php if($article['public'] == 1) echo 'selected="selected"'; ?> value="1">Chưa công khai</option>
        <option <?php if($article['public'] != 1) echo 'selected="selected"'; ?> value="2">Công khai</option>
    </select>
</div>
...
<script>
    ...
    $('.submit').click(function () {
        ...
        {
            $.ajax({
                url: baseUrl + 'backend/index/action?editarticle',
                type: 'POST',
        ...
        if (http.readyState == 4 && http.status == 200) {
            ...
            try { //Bẫy lỗi JSON
                ...
                if (server.status) {
                    ...
                    $('.upload').fadeOut();
                    $('.upload').find('img').attr('src', baseUrl+'upload/'+server.name);
                    $('.upload').fadeIn();
                } else {
